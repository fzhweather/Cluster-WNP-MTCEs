...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 25 10:36:34 2023
...
d3 = nc.Dataset(r'/DS1/xshome/fuzh19/nc/500gh.nc')                           
msl= d3.variables["z"][:]/98.0665
time=d1['time'][:]
u850=d1["u"][:]
v850=d2["v"][:]
...
def dyc1(t,ax):
    ...
    mslsum=msl[time==(int(data[0][12+t]))][:,70+round(float(data[0][5]/10)):111+round(float(data[0][5]/10)),round(float(data[0][6]/10))-20:round(float(data[0][6]/10))+61]
    cal.append(np.mean(msl[time==(int(data[0][12+t]))][:,85+round(float(data[0][5]/10)):96+round(float(data[0][5]/10)),round(float(data[0][6]/10))-5:round(float(data[0][6]/10))+6]))
    
    for i in range(1,len(data)):
        u850sum+=u850[time==(int(data[i][12+t]))][:,70+round(float(data[i][5]/10)):111+round(float(data[i][5]/10)),round(float(data[i][6]/10))-20:round(float(data[i][6]/10))+61]
        v850sum+=v850[time==(int(data[i][12+t]))][:,70+round(float(data[i][5]/10)):111+round(float(data[i][5]/10)),round(float(data[i][6]/10))-20:round(float(data[i][6]/10))+61]
        mslsum+=msl[time==(int(data[i][12+t]))][:,70+round(float(data[i][5]/10)):111+round(float(data[i][5]/10)),round(float(data[i][6]/10))-20:round(float(data[i][6]/10))+61]
        cal.append(np.mean(msl[time==(int(data[i][12+t]))][:,85+round(float(data[i][5]/10)):96+round(float(data[i][5]/10)),round(float(data[i][6]/10))-5:round(float(data[i][6]/10))+6]))
    u850mean=u850sum/len(data)
    v850mean=v850sum/len(data)
    mslmean=mslsum/len(data)   
    u_all=(np.mean(u850mean,axis=0))
    v_all=(np.mean(v850mean,axis=0))
    msl_all=(np.mean(mslmean,axis=0))
    levels = np.arange(582,590+0.01,0.5)
    cb = ax.contourf(lon2d,lat2d,msl_all,levels=levels,cmap=cmaps.BlueDarkRed18,extend='both')
    ax.contour(lon2d,lat2d,msl_all,[588],alpha=1,colors=['white'])

    for i in range(len(lat2d[::2])):
        for j in range(len(lon2d[::2])):
            if ((u_all[2*i,2*j])**2+(v_all[2*i,2*j])**2) >= 25:
                ax.quiver(lon2d[2*j],lat2d[2*i],u_all[2*i,2*j],v_all[2*i,2*j],color='k',scale=130,width=0.004)
            else:
                hh=ax.quiver(lon2d[2*j],lat2d[2*i],u_all[2*i,2*j],v_all[2*i,2*j],color='darkgray',scale=130,width=0.004)
    ...
    ax.quiverkey(hh, X=0.83, Y = 1.05, U = 10,angle = 0,label='10m/s',labelpos='E', color = 'k',labelcolor = 'k')
...
def dyc2(t,ax):
    ...
    mslsum=msl[time==(int(data[0][12+t]))][:,70+round(float(data[0][5]/10)):111+round(float(data[0][5]/10)),round(float(data[0][6]/10))-40:round(float(data[0][6]/10))+21]
    cal.append(np.mean(msl[time==(int(data[0][12+t]))][:,85+round(float(data[0][5]/10)):96+round(float(data[0][5]/10)),round(float(data[0][6]/10))-5:round(float(data[0][6]/10))+6]))
    for i in range(1,len(data)):
        ...
        v850sum+=v850[time==(int(data[i][12+t]))][:,70+round(float(data[i][5]/10)):111+round(float(data[i][5]/10)),round(float(data[i][6]/10))-40:round(float(data[i][6]/10))+21]
        mslsum+=msl[time==(int(data[i][12+t]))][:,70+round(float(data[i][5]/10)):111+round(float(data[i][5]/10)),round(float(data[i][6]/10))-40:round(float(data[i][6]/10))+21]
        cal.append(np.mean(msl[time==(int(data[i][12+t]))][:,85+round(float(data[i][5]/10)):96+round(float(data[i][5]/10)),round(float(data[i][6]/10))-5:round(float(data[i][6]/10))+6]))
    u850mean=u850sum/len(data)
    v850mean=v850sum/len(data)
    mslmean=mslsum/len(data)   
...
def dyc3(t,ax):
    ...
    if t==0:
        ...
        cbar=fig.colorbar(cb,cax=position,orientation='horizontal',ticks=np.arange(582,590+0.01,1),
...
np.save('./gh.npy',gh)
plt.savefig('dyn1.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
