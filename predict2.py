...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  3 15:47:08 2024
...
mtce = [0,	2,	1,	2,	1,	5,	5,	4,	2,	4,	2,	8,	3,	4,	2,	3,	2,	3,	2,	1,	0,	2,	5,	2,	2,	3,	2,	3,	0,	2,	3,	0,	1,	1,	1,	1,	4,	3,	1,	2,	3,	1]
mtce = [8,	4,	0,	3,	3,	2,	3,	3,	4,	3,	4,	3,	1,	3,	2,	4,	2,	1,	3,	0,	5,	5,	1,	1,	3,	2,	2,	0,	5,	4,	5,	1,	5,	3,	5,	0,	0,	5,	4,	2,	3,	6]
#mtce = [2,	4,	6,	5,	1,	3,	1,	3,	2,	0,	3,	2,	4,	8,	1,	7,	2,	3,	3,	0,	1,	4,	1,	4,	0,	8,	2,	2,	3,	0,	0,	0,	3,	2,	2,	4,	4,	1,	1,	4,	2,	0]
mtce = np.array(mtce)
...
for i in range(42):
  ...
  fitted.append(2.4803+0.1735*x['x1'][i]-1.3446*x['x2'][i]+2.951+0.736*x['x3'][i]-0.1563*x['x4'][i]+3.0365+0.113*x['x5'][i]+0.8403*x['x6'][i]-2.3483*x['x7'][i])  
...
dx = 0.32
dy = 0.3
xl = ['Nino3.4','Reconstructions']
yl = ['MTCE frequency','MTCE frequency']

for i in range(2):
    ax.append(fig.add_axes([x1[i],yy[i],dx,dy]))
tit=['(a) ENSO & MTCE','(b) MLR']
...
for i in range(2):
    ax[i].text(-2.2,2.86,tit[i])
    ax[i].text(-1.6,2.2,words[i])
    ...
    ax[i].scatter(x[i],y[i],s=18,color='gray',linewidth=0)
...
plt.savefig('/Users/fuzhenghang/Documents/python/WNP-MTCE/epsfig/MLR_ENSO.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
