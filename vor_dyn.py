...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec 25 17:24:00 2023
...
def dyc1(t,ax):
    ...
    levels = np.arange(-3,3+0.01,0.3)
    cb = ax.contourf(lon2d,lat2d,u_all,levels=levels,cmap='bwr',extend='both')
    ...
    ax.text(0,0,'+',color='black',weight="bold",size=15,horizontalalignment='center', verticalalignment='center')
    if t!=0:
        ax.set_xticklabels([])
...
def dyc3(t,ax):
    ...
    if t==0:
        ...
        cbar=fig.colorbar(cb,cax=position,orientation='horizontal',ticks=np.arange(-3,3+0.001,1),
...
np.save('./vor.npy',gh)
plt.savefig('dyn3.eps', format="eps", bbox_inches = 'tight',pad_inches=0,dpi=600)
